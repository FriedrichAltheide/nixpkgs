...
if [ ! "$oldVersion" = "$latestVersion" ]; then
  ...
  guestAdditionsIsoNixFile=$(nixFile  ${attr}GuestAdditionsIso)

  extpackOldShaSum=$(oldHash ${attr}Extpack)
  guestAdditionsIsoOldShaSum=$(oldHash ${attr}GuestAdditionsIso.src)
  ...
  sed -i -e 's|sha256 = "'$guestAdditionsIsoOldShaSum'"|sha256 = "'$guestAdditionsIsoShaSum'"|' $guestAdditionsIsoNixFile

  git add $virtualboxNixFile $extpackNixFile $guestAdditionsIsoNixFile
